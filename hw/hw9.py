...
def tree_to_ordered_sequence(s):
    ...
    if s is None:
        return Rlist.empty
    if s.left is None:
        return adjoin_set2(tree_to_ordered_sequence(s.right), s.entry)
    elif s.right is None:
        return adjoin_set2(tree_to_ordered_sequence(s.left), s.entry)
    else:
        return adjoin_set2(union_set2(tree_to_ordered_sequence(s.left), tree_to_ordered_sequence(s.right)), s.entry)
